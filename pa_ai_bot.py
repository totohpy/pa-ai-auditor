...
def init_state():
    ...
    # *** ข้อความต้อนรับใหม่ตามคำขอของผู้ใช้ ***
...
if prompt := st.chat_input("สอบถามเกี่ยวกับการตรวจสอบ หรือเอกสารที่โหลด/อัปโหลดไว้..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("กำลังประมวลผลคำตอบ..."):
            try:
                # 1. Prepare OpenAI Client (using OpenTyphoon)
                client = OpenAI(
                    api_key=api_key,
                    base_url="https://api.opentyphoon.ai/v1" # Base URL สำหรับ OpenTyphoon
                )
...
"""
                ...
                response_stream = client.chat.completions.create(
                    model="typhoon-v2.1-12b-instruct", # ใช้โมเดล OpenTyphoon
